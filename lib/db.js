...
-- Offizielle Torjaegerliste des Datenanbieters (Fallback, wenn sich aus den
-- gespeicherten Toren keine Schuetzen ableiten lassen, z.B. WM bei OpenLigaDB)
CREATE TABLE IF NOT EXISTS top_scorers (
  ...
  penalties       INTEGER,
  updated_at      TEXT NOT NULL DEFAULT (datetime('now')),
  PRIMARY KEY (league_shortcut, season, name)
);
`);

// Migrationen fuer bestehende Datenbanken
const leagueCols = db.prepare('PRAGMA table_info(leagues)').all().map((c) => c.name);
if (!leagueCols.includes('provider')) {
  db.exec("ALTER TABLE leagues ADD COLUMN provider TEXT NOT NULL DEFAULT 'openligadb'");
}
if (!leagueCols.includes('provider_league_id')) {
  db.exec('ALTER TABLE leagues ADD COLUMN provider_league_id TEXT');
}
