...
const hasEvent = db.prepare('SELECT 1 FROM events WHERE match_id = ? AND type = ? LIMIT 1');
...
async function processMatch(match, { silent = false } = {}) {
  ...
  // Halbzeit nicht am blossen Auftauchen des Halbzeitstands festmachen:
  // OpenLigaDB legt den Eintrag oft schon beim Anpfiff mit 0:0 an, was sonst
  // wenige Minuten nach Spielbeginn eine Halbzeit-Meldung ausloest. Statt-
  // dessen muss wirklich rund eine Halbzeit gespielt sein; gegen doppelte
  // Meldungen dient der bereits vorhandene Ticker-Eintrag.
  const playedMs = kickoffMs != null ? Date.now() - kickoffMs : null;
  if (
    !first && match.htScore1 != null && !match.isFinished &&
    playedMs != null && playedMs >= 45 * 60 * 1000 &&
    !hasEvent.get(match.id, 'halftime')
  ) {
    pending.push({
      ...
      // spaet bemerkt (z. B. nach Neustart): nur eintragen, nicht pushen
      noPush: playedMs > 80 * 60 * 1000,
    });
  }
